...
- 带 `test_enemy` 标签的测试敌人，在开战时应标准化回模板满状态，避免继承上一场残血
- 所有 `skill_card` 都必须统一遵守：
  - 扣除对应灵力
  - 进入对应冷却
- 基础测试敌人的 AI 不应连续空转叠 `防御`
  - 已有 `guard` 状态时，应优先切回攻击类卡牌
  - `guard` 在成功承接一次普通攻击后应被消耗
...
战斗文本展示优先服务“单回合理解”，而不是把底层字段原样堆给玩家。
...
- 每个回合都要独立可读：
  - 玩家回合如此
  - 敌方回合同样如此
  - 回合内至少包含出手前状态、所用卡牌、结算结果、出手后状态
- 我方与敌方状态分区展示，不再只按原始参战列表平铺
- 关键资源统一使用中文语义：
  - 气血
  - 灵力
  - 体力
